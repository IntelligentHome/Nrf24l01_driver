...
TEST(Nrf24l01_SendData, SendData) {
    ...
    const uint8_t sent_expected[] = {
        ...
        0x20, 0x0E, // Power up, TX mode
        0x00, 0x00,
    };

    const uint8_t get_expected[] = {
        0xFF, 0x0D, // Get Config (Power Down, RX mode)
        0x00, 0x00,
    };

    test_mocks::GpioMock mock_gpio(&set_cnt, &clear_cnt, &toggle_cnt, &state);
